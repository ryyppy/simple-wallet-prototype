module MyAccounts = {
  ...
  let make = (~wallets: array<Wallet.t>, ~onWalletSelect=?) => {
    let items = Belt.Array.map(wallets, wallet => {
      let onClick = evt => {
        ReactEvent.Mouse.preventDefault(evt)
        Belt.Option.forEach(onWalletSelect, fn => fn(wallet))
      }
      <li onClick key=wallet.address className="py-2 text-white-90-tr">
        <a onClick className="cursor-pointer hover:text-white"> {React.string(wallet.name)} </a>
      </li>
    })
...
module MyPages = {
  ...
  let make = () => {
    ...
    let items = Belt.Array.map(pages, ((text, target)) => {
...
module ConnectedBox = {
  ...
  let make = (~status: status) => {
    let (color, networkName) = switch status {
    ...
    <div className="bg-white-10-tr">
      <div className={"w-4 h-4 " ++ color} /> {React.string(networkName)}
    </div>
  }
}
...
module FullSidebar = {
  ...
  let make = (~onBurgerButtonClick, ~wallets, ~onWalletSelect) =>
    <div className="h-full min-h-screen bg-blue-dark">
      ...
      <div className="pl-8 pr-4 pt-12"> <MyAccounts wallets onWalletSelect /> </div>
