...
module TransactionHistory = {
  let styledAmount = (~gotPaid, volume) => {
    ...
    let amount = gotPaid ? Belt.Float.toString(volume) : "- " ++ Belt.Float.toString(volume)
    <span className> {React.string(amount)} </span>
  }

  @react.component
  let make = (~wallet: Wallet.t, ~data: array<TezStats.Transaction.t>) => {
    let {Wallet.address: address} = wallet
    let items = Belt.Array.map(data, tr => {
      let {TezStats.Transaction.row_id: row_id, sender, receiver, volume} = tr
      switch receiver {
      | Some(receiver) =>
        if receiver === address || sender === address {
          let gotPaid = receiver === address

          let arrowColors = gotPaid ? "text-green bg-green-light" : "text-red bg-red-10-tr"

          let arrow =
            <span className={arrowColors ++ " inline-block rounded-lg py-2 px-2"}>
      ...
      <table className="w-full">
        <thead>
          <tr className="text-10 uppercase font-bold">
            ...
            <th className=thClass> {React.string("Amount " ++ j`ꜩ`)} </th>
          </tr>
        </thead>
...
module Placeholder = {
  ...
  let make = () => {
    ...
    let mockupRows = Belt.Array.mapWithIndex(["w-1/4", "w-2/3", "w-4/6"], (i, width) =>
      <tr key={Belt.Int.toString(i)}>
        <td colSpan=4 className="py-3 pl-4">
          <div className={width ++ " rounded-lg animate-pulse h-6 bg-grey-40"} />
        </td>
      </tr>
    )

    <div className="bg-white rounded-lg px-6 pt-3 pb-6">
      <h1 className="text-16 font-bold mb-6"> {React.string("Transaction History")} </h1>
      <table className="w-full">
        <thead>
          <tr className="text-10 uppercase font-bold">
            <th className=thClass> <Icon.UpDownArrow /> </th>
...
let make = (~wallet: Wallet.t) => {
  ...
  React.useEffect1(() => {
    ...
    let onError = code => setState(_ => Failed("Error code " ++ Belt.Int.toString(code)))

    let cancel = TezStats.Transaction.queryByAddress(~address=wallet.address, ~onDone, ~onError, ())
  ...
  <MainLayout>
    <div className="flex justify-center w-full px-8">
      <div className="w-full" style={ReactDOMStyle.make(~maxWidth="54rem", ())}> content </div>
    </div>
  </MainLayout>
}
